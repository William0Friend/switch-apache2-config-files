...
run_in_background() {
    {
        "$@" >/dev/null 2>&1 </dev/null &  # Run the command in the background, redirecting output and input
        pid=$!  # Get the process ID of the background command
        (sleep 1 && kill -0 $pid 2>/dev/null && disown) &  # Check if the process is still running, then detach it using disown
    } >/dev/null 2>&1
}
...
create_background_group() {
    {
        "$@" >/dev/null 2>&1 </dev/null &  # Run the command in the background, redirecting output and input
        pid=$!  # Get the process ID of the background command
        (sleep 1 && kill -0 $pid 2>/dev/null) &  # Check if the process is still running
    } >/dev/null 2>&1
}
